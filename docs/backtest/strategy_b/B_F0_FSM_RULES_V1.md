...
tick"이고, 체결가가 가리키는 bar는 신호 bar다.

`signal_ttl_minutes = 2`는 `ENTRY_SIGNALLED` 이후 **2 wall-clock 분** 동안 다음 actual observed
bar의 시가에서 체결할 수 있는 신호 유효 창을 뜻한다. 만료 경계는 **포함**이며, 경계 시각에
열린 actual bar는 그 bar가 관측 가능해진 이후 tick에서 판정되므로 **엔진 내부 tick 수와 TTL
분 수는 같지 않을 수 있다**(2026-09-21 정정, 14절).
...
규칙이 아니라 설명 문장만 고친 기록이다. 규칙을 고치는 것은 12절의 절차를 따른다.

### 2026-09-21 - 8.3의 "두 tick" 표현 정정

| 항목 | 값 |
| --- | --- |
| 이전 문장 | "signal TTL 2분은 엔진에게 두 tick을 준다" |
| 문제 | wall-clock TTL의 의미를 엔진 내부 tick 수에 잘못 묶었다 |
| 정정 | 8.3의 새 문단. TTL은 다음 actual bar 시가에서 체결 가능한 신호 유효 창이고 경계는 포함이며, 경계 봉은 관측 가능해진 뒤 tick에서 판정되므로 tick 수와 분 수는 같지 않을 수 있다 |

변경되지 않은 것:

```text
canonical JSON            변경 없음
canonical SHA256          a082b998...  그대로
signal_ttl_minutes        2  그대로
구현                      이 정정으로 바뀐 코드 없음
전략 임계값               하나도 바뀌지 않음
```

`b_fsm_rules_v1.sha256`이 덮는 대상은 `b_fsm_rules_v1.json`뿐이므로, 이 문서의 산문 수정은
어떤 기록된 체크섬에도 영향을 주지 않는다.

배경: B-E0가 체결 창을 `next_actual_bar.open_time <= signal_at + signal_ttl_minutes`(경계 포함)로
선언했고, 2026-09-21에 구현을 그 선언에 맞추면서 엔진의 체결 시도 tick이 세 개가 되었다. 옛
문장은 그 시점부터 사실과 달랐다. 자세한 내용은 `B_E0_BACKTEST_CONTRACT_V1.md` 4.2.3.
