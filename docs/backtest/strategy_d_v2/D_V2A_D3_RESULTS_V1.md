...
## 부록 A. 감사 수정 (2026-09-21, D-V2A-4 착수 중 발견)

이 문서가 기록한 run `dv2a3-eda7523e6203`은 집 PC 산출물이다. 2026-09-21에 회사 PC에서 D4의 부모를
만들기 위해 같은 코드로 D3를 다시 돌렸더니 **`future_query_mutation`이 FAIL**했다. date 333에서
`b0_changed`, 즉 "미래를 변조했더니 D 시점 좌표로만 만드는 B0가 움직였다"는 보고였다.

누출이 아니었다. 실측 결과 rank matrix는 비트 단위로 동일했고, 갈린 것은 `B0 = rank_matrix @ weights`의
마지막 비트뿐이다. 감사가 **66,300행 곱으로 만든 아티팩트 값**을 참조로 쓰고 **300행 곱으로 재계산한
값**과 `np.array_equal`로 비교하고 있었기 때문이다. BLAS는 피연산자 크기에 따라 합산 순서를 바꾸고,
date 333의 300행 중 1행이 **2 ULP(2.776e-17)** 어긋났다. 집 PC의 BLAS가 그 행을 같게 계산해서 §8.1이
"findings 0"으로 기록됐을 뿐, **감사 결과가 머신에 의존**하고 있었다.

수정(`d3.py`, 규칙·좌표·K·embargo 무변경):

1. 참조를 감사와 **같은 경로·같은 shape**로 만든다(`audit_baseline`).
2. 미래가 실제로 오염시킬 수 있는 **좌표 행렬을 비트 비교**한다(`coordinates_changed` 신설). B0는
   좌표의 결정적 함수이므로 좌표가 같으면 누출 정보를 더하지 않는다.
3. 아티팩트 값과 감사 참조의 격차를 `b0_artifact_max_ulp`로 **보고**한다(숨기지 않는다).

재실행 `dv2a3-850a238d9e7a` **PASS**, findings 0, `b0_artifact_max_ulp` 2, 양성대조 894/900 이동으로
§8.1 기록과 동일. 회귀 테스트 3개 추가(기존 테스트는 양쪽을 같은 크기로 계산해 이 결함을 놓쳤다).

§11의 "바이트 동일" 주장은 **같은 머신 안에서** 유효하다. 교차머신 재현 범위는
`D_V2A_D4_RESULTS_V1.md` §11에 측정값과 함께 정리했다.
