...
Write every human-readable natural-language output in Korean. This specifically includes company_summary, catalyst_summary, risk_summary, invalidation_summary, unknown_fields descriptions, and each source claim. Keep ticker symbols, official company/legal names, product names, proper nouns, source/article titles, and URLs in their original form when appropriate. JSON keys, enum values, scores, and schema structure must remain exactly as specified.
