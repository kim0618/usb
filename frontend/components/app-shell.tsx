...
export const navigationItems: NavigationItem[] = [
  ...
  { href: "/candidates", icon: "◎", label: "종목 분석", activePaths: ["/candidates", "/research"] },
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-background">
    ...
    <div className="lg:pl-60"><header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-line bg-header px-4 md:px-7"><div className="flex items-center gap-3"><button className="btn-muted lg:hidden" onClick={() => setOpen(!open)} aria-label="메뉴">☰</button><div><p className="label">미국주식 트레이딩 시스템</p><p className="text-sm font-medium text-foreground">{state.data ? etTime(state.data.system_time) : "연결 중…"}</p></div></div><div className="flex items-center gap-3"><div className="hidden items-center gap-3 sm:flex"><span className="text-xs text-muted">시장</span><StatusBadge value={state.data?.market.session || "UNKNOWN"} label={formatMarketSession(state.data?.market.session || "UNKNOWN")}/><span className="text-xs text-muted">시스템</span><StatusBadge value={runtime} label={formatRuntimeMode(runtime)}/></div><ThemeToggle/></div></header><main className="p-4 md:p-7 xl:p-9">{children}</main></div>
  </div>;
}
