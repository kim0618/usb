...
export function StatusBadge({ value }: { value: string }) {
  ...
  return <span className={`inline-flex whitespace-nowrap rounded-full border px-2.5 py-1 text-[11px] font-bold tracking-wide ${tone}`}>{value}</span>;
}
export function RuntimeBanner({ mode }: { mode: RuntimeMode }) {
  if (mode === "NORMAL") return null;
  return <div className={`border-b px-5 py-2.5 text-center text-sm font-semibold ${mode === "HALTED" ? "border-red-700 bg-red-950/90 text-red-200" : "border-amber-700 bg-amber-950/90 text-amber-100"}`}>{mode === "HALTED" ? "HALTED — 자동 전략 Action이 중단되었습니다." : "SAFE MODE — 신규 진입과 추가매수가 차단되었습니다."}</div>;
}
...
export function EmptyState({ title, description }: { title: string; description?: string }) { return <div className="panel px-6 py-12 text-center"><p className="text-sm font-medium text-slate-300">{title}</p>{description && <p className="mt-2 text-xs text-muted">{description}</p>}</div>; }
...
export function ErrorState({ message, retry }: { message: string; retry?: () => void }) {
  ...
  if (missingResearch) return <EmptyState title="No GPT research imported." description="수동 GPT 분석 JSON을 Import하면 결과가 표시됩니다."/>;
  return <div className="panel border-red-900/70 p-6"><p className="font-semibold text-red-300">Backend unavailable</p><p className="mt-2 text-sm text-slate-400">{message}</p>{retry && <button className="btn-muted mt-4" onClick={retry}>다시 시도</button>}</div>;
}
