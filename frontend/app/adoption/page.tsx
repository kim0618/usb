...
export default function AdoptionPage() {
  ...
  return <><AnalysisTabs/>{result.loading ? <LoadingState/> : result.error && !noAnalysis ? <ErrorState message={result.error} retry={result.refresh}/> : !result.data ? <EmptyState title="채택 후보" description="GPT 분석이 완료되면 채택 후보가 생성됩니다."/> : <>
    <header className="mb-4"><h1 className="text-lg font-semibold text-foreground">채택 후보</h1><p className="mt-1 text-sm text-foreground-secondary">Quant와 GPT 분석을 함께 비교해 최종 검토가 필요한 종목을 보여줍니다.</p><p className="mt-2 text-xs text-muted">채택 후보 {result.data.counts.adoption_candidate}개 · 검토 필요 {result.data.counts.review_required}개 · 최종 채택 {approved}/2 · {result.data.filter_version}</p><p className="mt-1 text-xs text-muted">순위는 검토 순서이며 채택 결정이 아닙니다. 최종 채택은 최대 2개까지 가능합니다.</p></header>
