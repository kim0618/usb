...
/** Current-run rows can switch the run's active analysis; previous runs' rows are read-only history. */
function HistoryRow({ item, onActivate, past = false }: { item: ResearchHistoryItem; onActivate: (item: ResearchHistoryItem) => void; past?: boolean }) {
  const badge = past
    ? <StatusBadge value={item.is_active ? "PAST_ACTIVE" : "PAST_INACTIVE"} label={item.is_active ? "당시 거래 기준" : "미사용 분석"} tone="neutral"/>
    : item.is_active ? <StatusBadge value="ACTIVE" label="이 ScannerRun의 현재 활성 분석" tone="success"/> : <><StatusBadge value="INACTIVE" label="비활성 분석"/><button className="btn-action-secondary-compact" onClick={() => onActivate(item)}>이 분석 사용</button></>;
  return <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-line px-3 py-2"><div className="text-xs text-muted"><span className="font-semibold text-foreground">Analysis #{item.id}</span> · 분석 {kstTime(item.analysis_at)} · 저장 {kstTime(item.imported_at)} · 후보 {item.candidate_count}개<div className="mt-1">{item.candidates.map(candidate => `#${candidate.gpt_rank} ${candidate.symbol}`).join(" · ") || "후보 없음"}</div></div><div className="flex items-center gap-2">{badge}</div></div>;
}

const shortDate = (value: string | null | undefined) => value ? value.slice(5).replace("-", "/") : "-";

export default function ResearchPage() {
  const result = useApi(api.research); const history = useApi(api.researchHistory); const authority = useApi(api.researchCurrent); const toast = useToast();
  ...
  // The current run is the latest COMPLETED ScannerRun, whether or not it has an analysis yet.
  const currentRunId = authority.data?.scanner_run_id ?? result.data?.analysis.scanner_run_id ?? null;
  ...
  const refreshAll = () => Promise.all([result.refresh(), history.refresh(), authority.refresh()]);
  async function importJson() {
    if (!raw.trim()) { toast("GPT JSON을 입력하세요.", true); return; }
    setBusy(true);
    try { const response = await api.importResearch(raw); toast(response.activated ? `새 분석 #${response.analysis_id}이 저장되어 현재 거래 기준으로 설정되었습니다.` : `새 분석 #${response.analysis_id}이 저장되었습니다. 현재 거래 기준은 Analysis #${response.active_analysis_id}입니다. 새 분석을 사용하려면 활성화하세요.`); setRaw(""); setShowImport(false); await refreshAll(); }
  ...
  const currentDate = shortDate(authority.data?.trading_date ?? result.data?.analysis.trading_date);
  const historySection = history.data && history.data.length > 0 && <section className="panel mb-5 p-4">{currentHistory.length > 0 && <><h2 className="font-semibold">{currentDate} 분석 이력</h2><p className="mt-1 text-xs text-muted">활성 표시는 현재 ScannerRun 안에서 거래에 사용하는 분석을 뜻합니다.</p><div className="mt-3 space-y-2">{currentHistory.map(item => <HistoryRow key={item.id} item={item} onActivate={setPendingActivation}/>)}</div></>}{otherHistory.length > 0 && <details className={currentHistory.length > 0 ? "mt-4" : undefined}><summary className="cursor-pointer text-xs font-semibold text-foreground-secondary">이전 분석 이력 {otherHistory.length}건</summary><p className="mt-2 text-xs text-muted">이전 ScannerRun의 분석은 기록으로만 표시하며 현재 진입 후보에 사용되지 않습니다.</p><div className="mt-3 space-y-4">{Object.entries(otherGroups).map(([tradingDate, items]) => <section key={tradingDate}><h3 className="mb-2 text-xs font-semibold text-muted">{tradingDate} ScannerRun 분석</h3><div className="space-y-2">{items.map(item => <HistoryRow key={item.id} item={item} onActivate={setPendingActivation} past/>)}</div></section>)}</div></details>}</section>;
  const noRun = authority.data?.status === "NO_SCANNER_RUN";
  ...
  </> : <><PageHeader title="GPT 분석" description="분석 결과 JSON을 입력하면 후보별 평가와 순위가 표시됩니다."/>
    {authority.data && !noRun && <p className="mb-4 text-xs text-muted">현재 기준 거래일: {currentDate} · 현재 GPT 분석: 없음</p>}
    {importArea}
    <div className="mb-5">{noRun ? <EmptyState title="오늘 스캐너 실행 결과가 없습니다." description="Scanner 실행이 완료되면 해당 ScannerRun의 GPT 분석을 입력할 수 있습니다."/> : authority.data ? <EmptyState title="현재 ScannerRun에 대한 GPT 분석이 아직 없습니다." description="위 입력 영역에서 현재 ScannerRun의 GPT 분석 JSON을 검증하고 적용하세요. 이전 분석은 아래 이력에만 표시됩니다."/> : <EmptyState title="아직 불러온 분석 결과가 없습니다." description="위 입력 영역에서 GPT 분석 JSON을 검증하고 적용하세요."/>}</div>
    {historySection}</>}
  ...
  async function activate(id: number) {
    setBusy(true);
    try { await api.activateResearch(id); toast(`Analysis #${id}을 현재 거래 기준으로 설정했습니다.`); setPendingActivation(null); await refreshAll(); }
